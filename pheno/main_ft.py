...
from pheno.datasets import PhenoFinetuneDataset
from pheno.trainers import BertTuner
...
if __name__ == "__main__":    
    ...
    if config['max_seq_len'] == 'auto':
        ds_TESSy = pd.read_pickle(BASE_DIR / data_dict['load_path'])
        max_seq_len = ds_TESSy['num_ab'].max() + 4 + 1
        max_NCBI_seq_len = ds_NCBI['num_ab'].max() + 2 + 1 
        assert max_seq_len >= max_NCBI_seq_len, "Maximum sequence length of NCBI exceeds that of the model. Please adjust max_seq_len in the model."
    else:
        max_seq_len = config['max_seq_len']
        if not max_seq_len:
            raise ValueError("max_seq_len must be specified if option 'auto' is not provided.") 
    ...
    for i, train_share in enumerate(train_shares):
        ...
        if args.exp_folder:
            ...
            p = Path(BASE_DIR / "results" / "pheno" / args.exp_folder)
        else:
            p = Path(BASE_DIR / "results" / "pheno")
        ...
        for j, (train_indices, val_indices) in enumerate(indices_list):
            ...
            bert = BERT(config, vocab_size, max_seq_len, len(antibiotics), pad_idx).to(device)
            tuner = BertTuner(
                config=config,
                model=bert,
                antibiotics=antibiotics,
            ...
            if not config_ft['no_pt']:
                tuner.load_model(Path(BASE_DIR / 'results' / 'pheno' / config_ft['model_path']))
